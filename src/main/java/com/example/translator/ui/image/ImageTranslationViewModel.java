...
public class ImageTranslationViewModel extends ViewModel {
    ...
    private MutableLiveData<String> _detectedLanguage = new MutableLiveData<>();
    public final LiveData<String> detectedLanguage = _detectedLanguage;
    ...
    // Observer for user preferences
    private Observer<UserPreferences> preferencesObserver;
    ...
    public ImageTranslationViewModel(UserRepository userRepository, LanguageRepository languageRepository, Context context) {
        ...
        // Setup observer for user preferences to sync speech rate
        preferencesObserver = preferences -> {
            if (preferences != null) {
                currentSpeechRate = preferences.getSpeechRate();
                speechService.setSpeechRate(currentSpeechRate);
                _speechRate.postValue(currentSpeechRate);
                Log.d(TAG, "Speech rate loaded from preferences: " + currentSpeechRate);
            }
        };

        // Observe user preferences to get initial speech rate
        this.userPreferences.observeForever(preferencesObserver);
        ...
        executor.execute(() -> {
            try {
                ...
                if (targetLanguage == null || targetLanguage.isEmpty()) {
                    Log.e(TAG, "Invalid target language: " + targetLanguage);
                    _errorMessage.postValue("Please select target language.");
                    _isLoading.postValue(false);
                    return;
                }
                ...
                textRecognitionService.recognizeTextFromBitmap(bitmap, new TextRecognitionService.TextRecognitionCallback() {
                    ...
                    public void onSuccess(String recognizedText) {
                        ...
                        // Step 3: Handle translation based on source language
                        if ("auto".equals(sourceLanguage)) {
                            // Auto-detect language and translate
                            detectAndTranslateText(cleanText, targetLanguage);
                            ...
                            translateWithSourceLanguage(cleanText, sourceLanguage, targetLanguage);
                        }
                    }
     ...
     * Auto-detect language and translate
     */
    private void detectAndTranslateText(String text, String targetLanguage) {
        Log.d(TAG, "Auto-detecting language and translating to " + targetLanguage);

        translationService.detectLanguage(text, new TranslationService.LanguageDetectionCallback() {
            @Override
            public void onSuccess(String detectedLanguageCode) {
                Log.d(TAG, "Language detected: " + detectedLanguageCode);
                _detectedLanguage.postValue(detectedLanguageCode);

                if (detectedLanguageCode.equals(targetLanguage)) {
                    Log.d(TAG, "Detected language same as target, skipping translation");
                    _translationResult.postValue(text);
                    _isLoading.postValue(false);
                } else {
                    translateWithSourceLanguage(text, detectedLanguageCode, targetLanguage);
                }
            }
            ...
            public void onFailure(Exception exception) {
                ...
                // Fallback: try with English as source
                Log.d(TAG, "Falling back to English as source language");
                _detectedLanguage.postValue("en");

                if ("en".equals(targetLanguage)) {
                    _translationResult.postValue(text);
                    _isLoading.postValue(false);
                } else {
                    translateWithSourceLanguage(text, "en", targetLanguage);
                }
            }
        });
    }
     ...
     */
    private void translateWithSourceLanguage(String text, String sourceLanguage, String targetLanguage) {
    ...
    public void setSpeechRate(float rate) {
        ...
        executor.execute(() -> {
            try {
                UserPreferences currentPrefs = userPreferences.getValue();
                if (currentPrefs != null) {
                    currentPrefs.setSpeechRate(currentSpeechRate);
                    userRepository.updateUserPreferences(currentPrefs);
                    Log.d(TAG, "Speech rate saved to database: " + currentSpeechRate);
                }
            } catch (Exception e) {
                Log.e(TAG, "Error saving speech rate to database", e);
            }
        });
    }
    ...
    public void clearResults() {
        ...
        _detectedLanguage.setValue(null);
    ...
    protected void onCleared() {
        ...
        if (preferencesObserver != null && userPreferences != null) {
            userPreferences.removeObserver(preferencesObserver);
        }
