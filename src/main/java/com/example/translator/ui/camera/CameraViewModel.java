...
public class CameraViewModel extends ViewModel {
    ...
    /**
     * Tự động nhận diện ngôn ngữ và dịch
     */
    public void detectAndTranslateText(String text, String targetLanguage) {
        if (text == null || text.trim().isEmpty()) {
            Log.w(TAG, "No text to detect and translate");
            return;
        }
        ...
        executor.execute(() -> {
            try {
                ...
                Log.d(TAG, "Auto-detecting language for text: " + text.substring(0, Math.min(50, text.length())) + "...");

                // Bước 1: Nhận diện ngôn ngữ
                translationService.detectLanguage(text, new TranslationService.LanguageDetectionCallback() {
                    ...
                    public void onSuccess(String detectedLanguageCode) {
                        ...
                        // Bước 2: Dịch với ngôn ngữ đã nhận diện
                        if (detectedLanguageCode.equals(targetLanguage)) {
                            Log.d(TAG, "Detected language same as target, skipping translation");
                            _translationResult.postValue(text);
                            _isLoading.postValue(false);
                        } else {
                            Log.d(TAG, "Translating from detected language: " + detectedLanguageCode + " to " + targetLanguage);

                            translationService.translateText(text, detectedLanguageCode, targetLanguage,
                                    new TranslationService.TranslationCallback() {
                                        @Override
                                        public void onSuccess(String translatedText) {
                                            Log.d(TAG, "Auto-translation successful");
                                            _translationResult.postValue(translatedText);
                                            _isLoading.postValue(false);
                                        }
                                        ...
                                        @Override
                                        public void onFailure(Exception exception) {
                                            Log.e(TAG, "Auto-translation failed", exception);
                                            _translationResult.postValue(null);
                                            handleError("Translation failed", exception);
                                            _isLoading.postValue(false);
                                        }
                    ...
                    public void onFailure(Exception exception) {
                        ...
                        // Fallback: thử dịch với English làm source language mặc định
                        ...
                        if ("en".equals(targetLanguage)) {
                            ...
                            translationService.translateText(text, "en", targetLanguage,
                                    new TranslationService.TranslationCallback() {
                                        @Override
                                        public void onSuccess(String translatedText) {
                                            Log.d(TAG, "Fallback translation successful");
                                            _translationResult.postValue(translatedText);
                                            _isLoading.postValue(false);
                                        }

                                        @Override
                                        public void onFailure(Exception translationException) {
                                            Log.e(TAG, "Fallback translation failed", translationException);
                                            handleError("Language detection and translation failed", translationException);
                                            _isLoading.postValue(false);
                                        }
                                    });
                        }
                ...
                Log.e(TAG, "Error in auto-detect and translate process", e);
                handleError("Auto-translation failed", e);
                _isLoading.postValue(false);
            }
        });
    }
    ...
    public void detectLanguage(String text) {
        if (text == null || text.trim().isEmpty()) {
            Log.w(TAG, "No text to detect language");
            return;
        }

        executor.execute(() -> {
            try {
                Log.d(TAG, "Detecting language for: " + text.substring(0, Math.min(30, text.length())) + "...");

                translationService.detectLanguage(text, new TranslationService.LanguageDetectionCallback() {
                    @Override
                    public void onSuccess(String detectedLanguageCode) {
                        Log.d(TAG, "Language detection successful: " + detectedLanguageCode);
                        _detectedLanguage.postValue(detectedLanguageCode);
                    }
                    ...
                    public void onFailure(Exception exception) {
                        Log.e(TAG, "Language detection failed", exception);
                        _detectedLanguage.postValue("und"); // undetermined
                    }
                });

            } catch (Exception e) {
                Log.e(TAG, "Error in language detection", e);
                _detectedLanguage.postValue("und");
            }
        });
    }
    ...
    // Method to get detected language
    public String getDetectedLanguageValue() {
        return _detectedLanguage.getValue();
    }
